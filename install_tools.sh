...
apt-get install -y --no-install-recommends locales procps lsb-release vim pwgen jq wget curl unzip gpg gettext ca-certificates openssh-client git bzip2
...
echo "deb [ signed-by=/usr/share/keyrings/mongodb-server-8.0.gpg ] http://repo.mongodb.org/apt/debian bookworm/mongodb-org/8.2 main" | tee /etc/apt/sources.list.d/mongodb-org-8.2.list
...
apt-get update >/dev/null
apt-get install -y --no-install-recommends skopeo htop netcat-openbsd uuid-runtime dnsutils eza fd-find trivy iproute2 nmap iperf3 \
